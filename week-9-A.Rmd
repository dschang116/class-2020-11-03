...
```{r}
fit_obj_1 <- stan_glm(formula = poverty ~ distance,
                    data = week_9,
                    refresh = 0)
```
...
$$reg\_chg_i = \beta_0 + beta_1 treatment_i + \beta_2 poverty_i \\ + beta_3 distance_i + \beta_4 pop\_density_i + \epsilon_i$$

```{r}
fit_obj_2 <- stan_glm(formula = reg_chg ~ treatment + poverty + distance + pop_density,
         data = week_9,
         refresh = 0)
```

```{r}
print(fit_obj_2, digits = 5)
```
